...
const EntregasLocal = ({ navigation, route }) => {
  const { sharedElementPrefix } = route.params;
  const traslados = useSelector((state) => state.traslados.trasladosLocal);
  ...
  useEffect(() => {
    if (isFocused) {
      dispatch(
        countTrasladosLocal(data?.nombre, data?.apellido, null, user?.username)
      );
    }
  }, [dispatch, isFocused]);
  ...
  function resetFilter() {
    ...
    dispatch(countTrasladosLocal("", "", null, user?.username));
  }
...
EntregasLocal.sharedElement = (route, otherRoute, showing) => {
  if (otherRoute.name === "Dashboard") {
    const { sharedElementPrefix } = route.params;
    return [
      {
        id: `${sharedElementPrefix}-ToolsCard-Bg-EntregasLocal`,
      },
      {
        id: `${sharedElementPrefix}-ToolsCard-Title-EntregasLocal`,
      },
    ];
  }
};

export default EntregasLocal;
