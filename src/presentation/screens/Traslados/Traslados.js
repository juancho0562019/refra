...
const Traslados = ({ navigation, route }) => {
  const { itemFecha, sharedElementPrefix, action } = route?.params;

  const herramientas = useSelector((state) => state.productos.productos);
  const conductores = useSelector((state) => state.conductores.conductores);
  const bodegas = useSelector((state) => state.bodegas.bodegas);
  ...
  function articulosHandler() {
    navigation.navigate("ArticulosSeleccionados", {
      itemFecha: itemFecha,
      listHerramientas: list,
      bodegaOrigen: user?.bodegaId,
      addItem: data?.codInterno > 0 ? false : true,
    });
  }

  function saveTraslado(fecha) {
    saveTrasladoEntregaLocal(
      "",
      user.username,
      data.conductor,
  ...
  async function saveTrasladoAPI() {
    let dataSend = {
      ...
      detalles: [],
      fecha: data?.fecha,
      bodegaOrigenId: parseInt(user?.bodegaId),
      bodegaDestinoId: parseInt(data?.bodegaDestino),
    };
    ...
    //const dataEnvio = await APIHandler.post(`${Globals.SaveTraslado}`, dataSend).toPromise();
    ...
    if (!netInfo.isConnected) {
      ...
      if (isSaved) {
        (async () => {
          const dataEnvio = await APIHandler.post(
            `${Globals.SaveTraslado}`,
            dataSend
          )
            .toPromise()
            .then((response) => {
              updateCodInterno(data?.id, response, user.username);
              setData({
                ...data,
                codInterno: response,
              });
        ...
        {loading ? (
          ...
          <Animatable.View duration={500} delay={1050} style={{ flex: 1 }}>
            ...
            >
              ...
              {data?.id > 0 && data?.codInterno <= 0 && !loading ? (
                <Animatable.View
              ...
              {data?.codInterno > 0 ? null : (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {data?.id > 0 ? "Actualizar" : "Realizar traslado"}{" "}
                    </Text>
                  </TouchableOpacity>
                </Animatable.View>
              )}
              <Animatable.View
                ...
                duration={200}
                delay={1050}
                style={{ marginLeft: 19, flexDirection: "row" }}
              >
                <Text>Los campos marcados con (</Text>
